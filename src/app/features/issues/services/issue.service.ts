...
export class IssueService extends ResourceService<Issue> {
  ...
  getAllMeetingIssues(): Observable<Issue[]> {
    return this.http.get<Issue[]>(`${this.API_URL}/allMeetingIssue`);
  }

  getAllUpcomingIssues(): Observable<Issue[]> {
    return this.http.get<Issue[]>(`${this.API_URL}/allUpcommingIssues`);
  }
}
