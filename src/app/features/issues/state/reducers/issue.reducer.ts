...
export const reducer = createReducer<State>(
  ...
  on(IssueApiActions.updateIssueSuccess, (state, action): State => {
    const updatedIssues = state.issues.map((issue) =>
      action.issue.id === issue.id ? action.issue : issue
    );
    return {
      ...state,
      issues: updatedIssues,
  ...
  on(IssueApiActions.loadMeetingIssuesSuccess, (state, action): State => {
    return {
      ...state,
      meetingIssues: action.meetingIssues,
      error: null,
    };
  }),
  on(IssueApiActions.loadUpcomingIssuesSuccess, (state, action): State => {
    return {
      ...state,
      upcommingIssues: action.upcommingIssues,
      error: null,
    };
  })
);
