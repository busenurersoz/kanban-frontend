...
export class IssueEffects {
  ...
  loadMeetingIssues$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(IssuePageActions.loadMeetingIssues),
      mergeMap((action) => {
        return this.issueService.getAllMeetingIssues().pipe(
          map((issues) =>
            IssueApiActions.loadMeetingIssuesSuccess({
              meetingIssues: issues,
            })
          ),
          catchError((error) =>
            of(IssueApiActions.loadIssuesFailure({ error: error.message }))
  ...
  loadUpcommingIssues$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(IssuePageActions.loadUpcommingIssues),
      mergeMap((action) => {
        return this.issueService.getAllUpcomingIssues().pipe(
          map((issues) =>
            IssueApiActions.loadUpcomingIssuesSuccess({
              upcommingIssues: issues,
            })
          ),
          catchError((error) =>
            of(IssueApiActions.loadIssuesFailure({ error: error.message }))
